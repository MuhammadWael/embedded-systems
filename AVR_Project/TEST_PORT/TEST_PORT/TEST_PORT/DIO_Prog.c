...
u8 DIO_u8_Get_pin_val(u8 copy_u8_port, u8 copy_u8_pin)
{
	...
	if (copy_u8_pin <= 7 &&copy_u8_pin >=0)
	{
		switch(copy_u8_port)
		{
			...
			case PORTB: local_u8_val = GET_BIT(PINA_REG,copy_u8_pin);break;
			case PORTC: local_u8_val = GET_BIT(PINA_REG,copy_u8_pin);break;
			case PORTD: local_u8_val = GET_BIT(PINA_REG,copy_u8_pin);break;
